...
class LidarToCameraFOV(Node):
    def __init__(self):
        ...
        # Camera parameters
        self.f_x = 277.0  # From CameraInfo
        self.f_y = 277.0
        self.image_width = 320
        self.image_height = 240

        # Precompute camera FOV
        self.horizontal_fov = 2 * np.arctan(self.image_width / (2 * self.f_x))
        self.vertical_fov = 2 * np.arctan(self.image_height / (2 * self.f_y))

        # LIDAR alignment adjustment (9 o'clock to 12 o'clock)
        self.lidar_alignment_offset = -np.pi / 2  # -90 degrees in radians

    def lidar_callback(self, msg):
        # Calculate total number of ranges
        total_ranges = len(msg.ranges)

        # Calculate angle bounds of the camera's FOV in the LIDAR frame
        center_angle = 0.0 + self.lidar_alignment_offset  # Adjusted center
        ...
        #     f"Horizontal FOV (rad): {self.horizontal_fov:.2f}, "
        #     f"Left index: {left_index}, Right index: {right_index}, "
        #     f"Front ranges count: {len(front_ranges)}"
        # )
